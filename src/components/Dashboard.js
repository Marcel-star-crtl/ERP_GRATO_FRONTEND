...
const Dashboard = () => {
 ...
 const fetchDashboardStats = async () => {
    try {
      ...
      const apiCalls = [];
      let communicationsIndex = -1;
      let inventoryIndex = -1;
      let assetIndex = -1;
      let supplierIndex = -1;
      let budgetCodesIndex = -1;
      let projectsIndex = -1;
      let incidentsIndex = -1;
      let sharepointIndex = -1;

      // Budget code stats for finance
      if (user?.role === 'finance' || user?.role === 'admin') {
        budgetCodesIndex = apiCalls.length;
        apiCalls.push(
      ...
      // Project stats for all users
      projectsIndex = apiCalls.length;
      apiCalls.push(
        api.get('/projects/dashboard-stats')
          .catch(() => ({ data: { pending: 0, inProgress: 0, completed: 0, total: 0 } }))
      );

      // Incident stats for all users
      incidentsIndex = apiCalls.length;
      apiCalls.push(
        api.get('/incident-reports/dashboard-stats')
          .catch(() => ({ data: { pending: 0, total: 0 } }))
      );

      // SharePoint stats for all users
      sharepointIndex = apiCalls.length;
      apiCalls.push(
        api.get('/sharepoint/dashboard-stats')
          .catch(() => ({ data: { pending: 0, total: 0 } }))
      );

      // Communications stats for admin and hr
      if (user?.role === 'admin' || user?.role === 'hr') {
        communicationsIndex = apiCalls.length;
        apiCalls.push(
      ...
      if (user?.role === 'supervisor') {
        apiCalls.push(api.get('/cash-requests/supervisor/stats').catch(() => ({ data: { pending: 0, total: 0 } })));
        apiCalls.push(api.get('/it-support/supervisor').catch(() => ({ data: [] })));
        apiCalls.push(api.get('/sick-leave/supervisor/stats').catch(() => ({ data: { pending: 0, total: 0 } })));
        apiCalls.push(api.get('/suggestions/supervisor/stats').catch(() => ({ data: { pending: 0, total: 0 } })));
      } else {
        apiCalls.push(api.get('/cash-requests/dashboard-stats').catch(() => ({ data: { pending: 0, total: 0 } })));
        apiCalls.push(api.get('/it-support/dashboard/stats').catch(() => ({ data: { summary: { pending: 0, total: 0 } } })));
        apiCalls.push(api.get('/sick-leave/dashboard-stats').catch(() => ({ data: { pending: 0, total: 0 } })));
        apiCalls.push(api.get('/suggestions/dashboard-stats').catch(() => ({ data: { pending: 0, total: 0 } })));
      }

      // Inventory and Asset Stats for supply_chain and admin
      if (user?.role === 'supply_chain' || user?.role === 'admin') {
        inventoryIndex = apiCalls.length;
        apiCalls.push(
          api.get('/inventory/dashboard')
            .catch(() => ({ data: { data: { summary: { totalItems: 0, lowStockItems: 0, outOfStockItems: 0, totalStockValue: 0 } } } }))
        );

        assetIndex = apiCalls.length;
        apiCalls.push(
      ...
      // Parse project stats
      let projectStats = { pending: 0, inProgress: 0, completed: 0, total: 0 };
      if (projectsIndex !== -1 && responses[projectsIndex]?.status === 'fulfilled') {
        projectStats = responses[projectsIndex].value.data;
      }

      // Parse incident stats
      let incidentStats = { pending: 0, total: 0 };
      if (incidentsIndex !== -1 && responses[incidentsIndex]?.status === 'fulfilled') {
        incidentStats = responses[incidentsIndex].value.data;
      }

      // Parse SharePoint stats
      let sharepointStats = { pending: 0, total: 0 };
      if (sharepointIndex !== -1 && responses[sharepointIndex]?.status === 'fulfilled') {
        sharepointStats = responses[sharepointIndex].value.data;
      }

      // Parse communications stats if applicable
      let communicationsStats = { pending: 0, total: 0 };
      if (communicationsIndex !== -1 && responses[communicationsIndex]?.status === 'fulfilled') {
        const data = responses[communicationsIndex].value.data;
        communicationsStats = {
          pending: (data.drafts || 0) + (data.scheduled || 0),
          total: data.sent || 0
        };
      }

       // Parse budget codes stats if applicable
      let budgetCodesStats = { pending: 0, total: 0, revisions: 0, transfers: 0 };
      if (budgetCodesIndex !== -1 && responses[budgetCodesIndex]?.status === 'fulfilled') {
        const data = responses[budgetCodesIndex].value.data;
        budgetCodesStats = {
      ...
      // Adjust indices based on all new API calls
      const baseOffset = (projectsIndex !== -1 ? 1 : 0) + 
                         (incidentsIndex !== -1 ? 1 : 0) + 
                         (sharepointIndex !== -1 ? 1 : 0);
      const offset = communicationsIndex !== -1 ? baseOffset + 1 : baseOffset;
      
      const cashRequestsResponse = responses[offset];
      const itSupportResponse = responses[offset + 1];
      const sickLeaveResponse = responses[offset + 2];
      const suggestionsResponse = responses[offset + 3];
      ...
      let inventoryStats = { totalItems: 0, lowStockItems: 0, outOfStockItems: 0, totalStockValue: 0 };
      let assetStats = { totalAssets: 0, inUseAssets: 0, overdueInspections: 0, totalValue: 0 };
      let supplierStats = { totalSuppliers: 0, topPerformers: 0, averageScore: 0 };

      if (user?.role === 'supply_chain' || user?.role === 'admin') {
        if (inventoryIndex !== -1 && responses[inventoryIndex]?.status === 'fulfilled') {
          const data = responses[inventoryIndex].value.data.data.summary;
          inventoryStats = {
        ...
        if (assetIndex !== -1 && responses[assetIndex]?.status === 'fulfilled') {
          const data = responses[assetIndex].value.data.data;
          assetStats = {
            totalAssets: data.summary.totalAssets || 0,
            inUseAssets: data.summary.inUseAssets || 0,
            overdueInspections: data.summary.overdueInspections || 0,
            totalValue: data.valuation.totalCurrentValue || 0
          };
        }

        if (supplierIndex !== -1 && responses[supplierIndex]?.status === 'fulfilled') {
          const data = responses[supplierIndex].value.data.data;
          supplierStats = {
            totalSuppliers: data.rankings.length || 0,
            topPerformers: data.rankings.filter(s => s.performanceGrade === 'A').length || 0,
      ...
      setStats({
        ...
        fixedAssets: {
          ...
          inUse: assetStats.inUseAssets,
          overdueInspections: assetStats.overdueInspections,
          totalValue: assetStats.totalValue
        },
        supplierPerformance: {
          totalSuppliers: supplierStats.totalSuppliers,
          topPerformers: supplierStats.topPerformers,
          averageScore: supplierStats.averageScore
        },
        dataMigration: {
          pending: 0,
          completed: 0,
  ...
  const getModuleCards = () => {
    ...
    const modules = [
      ...
      //   key: 'debit-notes',
      //   title: 'Debit Notes',
      //   description: 'Manage debit notes for purchase orders',
      //   icon: <FileTextOutlined style={{ fontSize: '48px', color: '#f5222d' }} />,
      //   color: '#fff2f0',
      //   borderColor: '#f5222d',
      //   stats: {
      //     total: debitNoteStats.total,
      //     pending: debitNoteStats.pending,
      //     approved: debitNoteStats.approved
      //   },
      //   managementRoles: ['buyer', 'supply_chain', 'admin'],
      //   actions: {
      //     buyer: [
      //       { label: 'Manage Debit Notes', path: '/buyer/debit-notes', icon: <FileTextOutlined />, primary: true },
      //       { label: 'Create Debit Note', path: '/buyer/debit-notes/new', icon: <PlusOutlined /> }
      //     ],
      //     supervisor: [
...
//         communicationsIndex = apiCalls.length;
//         apiCalls.push(
//           api.get('/api/communications/stats/dashboard')
//             .catch(() => ({ data: { drafts: 0, scheduled: 0, sent: 0 } }))
//         );
//       }
...
//         assetIndex = apiCalls.length;
//         apiCalls.push(
//           api.get('/api/fixed-assets/dashboard')
//             .catch(() => ({ data: { data: { summary: { totalAssets: 0, inUseAssets: 0, overdueInspections: 0 }, valuation: { totalCurrentValue: 0 } } } }))
//         );
        
//         supplierIndex = apiCalls.length;
//         apiCalls.push(
//           api.get('/api/supplier-performance/rankings?limit=10')
//             .catch(() => ({ data: { data: { rankings: [], summary: { totalSuppliers: 0, averageScore: 0 } } } }))
//         );
//       }
...
//       // Parse other stats (adjust indices based on whether communications was added)
//       const offset = communicationsIndex !== -1 ? 1 : 0;
//       const cashRequestsResponse = responses[offset];
//       const itSupportResponse = responses[offset + 1];
//       const sickLeaveResponse = responses[offset + 2];
//       const suggestionsResponse = responses[offset + 3];
...
//         level: 3,
//         canView: ['all'],
//         canManage: ['purchase-requisitions', 'procurement', 'vendor-management', 'inventory', 'fixed-assets'],
//         canApprove: ['purchase-requisitions'],
//         hasTeamAccess: true
//       },
//       buyer: {
...
//         borderColor: '#faad14',
//         stats: stats.incidentReports,
//         managementRoles: ['hse', 'admin'], // Changed from 'hr'
//         actions: {
//           base: [
//             { label: 'My Reports', path: '/employee/incident-reports', icon: <UserOutlined /> },
//             { label: 'New Report', path: '/employee/incident-reports/new', icon: <ArrowRightOutlined /> }
...
//         actions: {
//           base: [
//             { label: 'My Tasks', path: '/employee/action-items', icon: <UserOutlined /> },
//             { label: 'New Task', path: '/employee/action-items/new', icon: <PlusOutlined /> }
//           ],
//           supply_chain: [
//             { label: 'Task Management', path: '/supply-chain/action-items', icon: <CrownOutlined />, primary: true, badge: true },
//             { label: 'Team Tasks', path: '/supply-chain/action-items?view=team', icon: <TeamOutlined /> },
...
//       supply_chain: {
//         title: 'Supply Chain Dashboard',
//         description: 'All services + procurement, inventory, assets and team vendor management',
//         icon: <ShoppingCartOutlined />,
//         color: '#fa8c16'
//       },
//       buyer: {
...
//     // Add inventory alerts for supply chain
//     if (user?.role === 'supply_chain' || user?.role === 'admin') {
//       return (
//         Object.values(stats).reduce((sum, stat) => sum + (stat.pending || 0), 0) +
//         (stats.inventory.lowStock || 0) +
...
//   const managementModules = ['pettycash', 'purchase-requisitions', 'buyer-procurement', 'supplier-management', 'invoices', 'incident-reports', 'it-support', 'suggestions', 'sick-leave', 'communications', 'inventory-management', 'fixed-assets', 'supplier-performance', 'data-migration']
//     .filter(module => {
//       const moduleConfig = {
//         'pettycash': ['finance', 'hr', 'admin'],
//         'purchase-requisitions': ['supply_chain', 'buyer', 'hr', 'it', 'finance', 'admin'],
...
//                   <br />
//                   <Text strong style={{ color: '#fa8c16' }}>Supply Chain Management:</Text>
//                   <Text type="secondary"> Full inventory control, asset registry, and supplier performance tracking.</Text>
//                 </>
//               )}
//             </div>
//           }
...
//             { label: 'New Task', path: '/employee/action-items/new', icon: <PlusOutlined /> },
//             { label: 'My KPIs', path: '/employee/kpis', icon: <TrophyOutlined /> },
//             { label: 'Performance Evaluation', path: '/employee/performance-evaluation', icon: <BarChartOutlined /> }
//           ],
//           supply_chain: [
//             { label: 'Task Management', path: '/supply-chain/action-items', icon: <CrownOutlined />, primary: true, badge: true },
//             { label: 'Team Tasks', path: '/supply-chain/action-items?view=team', icon: <TeamOutlined /> },
...
//             { label: 'Team Action Items', path: '/supervisor/action-items', icon: <TeamOutlined />, badge: true },
//             { label: 'KPI Approvals', path: '/supervisor/kpi-approvals', icon: <TrophyOutlined /> },
//             { label: 'Behavioral Evaluations', path: '/supervisor/behavioral-evaluations', icon: <StarOutlined /> },
//             { label: 'Quarterly Evaluations', path: '/supervisor/quarterly-evaluations', icon: <BarChartOutlined /> }
//           ],
//           finance: [
//             { label: 'Team Action Items', path: '/supervisor/action-items', icon: <TeamOutlined />, badge: true }
...
//         borderColor: '#13c2c2',
//         stats: {
//           totalItems: 0, // Will be populated from API
//           lowStock: 0,
...
//             { label: 'Inventory Dashboard', path: '/supply-chain/inventory', icon: <DashboardOutlined />, primary: true },
//             { label: 'Available Stock', path: '/supply-chain/inventory?tab=overview', icon: <DatabaseOutlined /> },
//             { label: 'Record Inbound', path: '/supply-chain/inventory/inbound', icon: <InboxOutlined />, badge: false },
//             { label: 'Record Outbound', path: '/supply-chain/inventory/outbound', icon: <ShoppingOutlined /> },
//             { label: 'Reorder Alerts', path: '/supply-chain/inventory?tab=alerts', icon: <WarningOutlined />, badge: true },
//             { label: 'Stock Reports', path: '/supply-chain/inventory?tab=reports', icon: <BarChartOutlined /> },
//             { label: 'Valuation Report', path: '/supply-chain/inventory/valuation', icon: <DollarOutlined /> }
...
//         borderColor: '#722ed1',
//         stats: {
//           totalAssets: 0, // Will be populated from API
//           inUse: 0,
...
//         borderColor: '#fa8c16',
//         stats: {
//           totalSuppliers: 0, // Will be populated from API
//           topPerformers: 0,
...
//   const managementModules = ['pettycash', 'purchase-requisitions', 'buyer-procurement', 'supplier-management', 'invoices', 'incident-reports', 'it-support', 'suggestions', 'sick-leave', 'communications']
//     .filter(module => {
//       const moduleConfig = {
//         'pettycash': ['finance', 'hr', 'admin'],
//         'purchase-requisitions': ['supply_chain', 'buyer', 'hr', 'it', 'finance', 'admin'],
